    ...
    unsigned char *more)
{
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (tmp[0] & BIT_FIFO_OVERFLOW) {
            mpu_reset_fifo();
////					LCD_ShowNum(30,320,fifo_count,32,16);
            return -2;
        }
    }
...
//	unsigned char more;
	long quat[4]; 
	int8_t error;
//	if(error = dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors,&more),error)
	if(error=MPU_FIFO_RADE(gyro, accel, quat, 0),error)	//自己写的读数据函数
		return 1;	 
